...
class EnvironmentProfile(BaseModel):
    name: str               # Friendly label, e.g. "Production Supabase"
    db_url: str             # Full PostgreSQL connection string
    description: Optional[str] = ""
    color: Optional[str] = "#6366f1"   # UI accent colour (hex)
    anon_key: Optional[str] = ""       # Supabase anon key (SUPABASE_ANON_KEY)
...
async def activate_environment(env_id: str):
    ...
    anon_key = target.get("anon_key", "")

    updates = {"DATABASE_URL": db_url}
    if anon_key:
        updates["SUPABASE_ANON_KEY"] = anon_key
