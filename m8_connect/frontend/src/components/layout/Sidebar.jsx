...
import { canAccessMenu, isConnectAdmin } from '../../utils/permissions';
...
const configChildren = [
  { path: '/config/catalogs', icon: BookOpen, label: 'Catálogos', permission: 'config.catalogs_view', adminAlways: true },
  { path: '/config/history', icon: History, label: 'Historia', permission: 'config.history_view', adminAlways: true },
  { path: '/config/roles', icon: Shield, label: 'Roles', adminOnly: true },
];
...
const Sidebar = () => {
  ...
  const visibleNavItems = useMemo(
    () => navItems.filter((item) => canAccessMenu(user, item.menuKey)),
    [user]
  );

  const visibleConfigChildren = useMemo(() => configChildren.filter((item) => {
      if (item.adminOnly) return isConnectAdmin(user);
      if (isConnectAdmin(user)) return true;
      if (item.permission) {
        const key = item.permission.split('.').pop();
        return user?.permissions?.config?.[key];
      }
      return false;
    }), [user]);

  const showConfigSection = visibleConfigChildren.length > 0;
  ...
  const topNav = visibleNavItems.filter((item) => item.path === '/' || item.path === '/upload');
  const bottomNav = visibleNavItems.filter((item) => item.path !== '/' && item.path !== '/upload');
        ...
        {showConfigSection && (
          <div className="mb-0.5">
            ...
            >
              ...
              <span className="hidden md:inline flex-1 text-left">Configuración</span>
              <ChevronDown
                size={14}
                className={cn(
            ...
            {configOpen && (
              <div className="mt-0.5 space-y-0.5 pl-2 md:pl-4">
                {visibleConfigChildren.map((item) => (
                  <NavLink
                    key={item.path}
                    to={item.path}
                    className={({ isActive }) =>
