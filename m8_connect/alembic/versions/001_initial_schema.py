...
def upgrade() -> None:
    ...
    # Create custom ENUM types in PostgreSQL using raw SQL
    op.execute("CREATE TYPE staging_meta.sourcetype AS ENUM ('file', 'api', 'database', 'stream', 'ftp', 'sftp', 's3', 'manual')")
    op.execute("CREATE TYPE staging_meta.batchstatus AS ENUM ('PENDING', 'PENDING_PROCESS', 'PENDING_MAPPING', 'PENDING_PREVIEW', 'UPLOADED', 'PROCESSING', 'VALIDATING', 'TRANSFORMING', 'LOADING', 'COMPLETED', 'FAILED', 'CANCELLED', 'RETRY', 'PROMOTED', 'PARTIALLY_PROMOTED')")
    op.execute("CREATE TYPE staging_meta.loadtype AS ENUM ('FULL', 'INCREMENTAL', 'DELTA', 'APPEND', 'REPLACE', 'MERGE', 'FILE_UPLOAD', 'API_SYNC', 'STREAM')")
    op.execute("CREATE TYPE staging_meta.loadstatus AS ENUM ('IN_PROGRESS', 'COMPLETED', 'FAILED', 'CANCELLED', 'PARTIALLY_COMPLETED')")
    op.execute("CREATE TYPE staging_meta.validationstatus AS ENUM ('PENDING', 'PASSED', 'FAILED', 'WARNING', 'ERROR', 'CRITICAL')")
    op.execute("CREATE TYPE staging_meta.validationtype AS ENUM ('completeness', 'uniqueness', 'data_type', 'range', 'pattern', 'business_rule', 'referential_integrity', 'custom', 'format', 'consistency')")
    ...
    batch_status_enum = postgresql.ENUM(
        'PENDING', 'PENDING_PROCESS', 'PENDING_MAPPING', 'PENDING_PREVIEW',
        'UPLOADED', 'PROCESSING', 'VALIDATING', 'TRANSFORMING', 
        'LOADING', 'COMPLETED', 'FAILED', 'CANCELLED', 'RETRY',
        'PROMOTED', 'PARTIALLY_PROMOTED',
        name='batchstatus',
        schema='staging_meta',
        create_type=False
    )
    ...
    summary_cols = [
        sa.Column('source_name', string_type(100), primary_key=True, nullable=False, comment='Name of the data source'),
    ...
    batch_control_cols = [
        sa.Column('batch_id', uuid_type, primary_key=True, default=uuid.uuid4, comment='Unique identifier for the batch'),
        ...
        sa.Column('organization_id', string_type(100), nullable=True, comment='Organization identifier'),
        sa.Column('created_at', datetime_type, server_default=sa.text('now()'), nullable=False, comment='Timestamp when record was created'),
        sa.Column('updated_at', datetime_type, server_default=sa.text('now()'), nullable=False, comment='Timestamp when record was last updated'),
    ]
    ...
    load_history_cols = [
        sa.Column('load_id', uuid_type, primary_key=True, default=uuid.uuid4, comment='Unique identifier for the load operation'),
    ...
    validation_logs_cols = [
        sa.Column('log_id', uuid_type, primary_key=True, default=uuid.uuid4, comment='Unique identifier for the validation log entry'),
    ...
    template_staging_cols = [
        sa.Column('staging_id', uuid_type, primary_key=True, default=uuid.uuid4, comment='Unique identifier for the staging record'),
