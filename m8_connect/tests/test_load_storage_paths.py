...
def test_build_history_storage_dir(tmp_path: Path):
    ...
    assert path == tmp_path / "acme-corp" / HISTORIA_DIR / "2026-06-12_143052"
...
def test_build_catalog_storage_dir(tmp_path: Path):
    ...
    assert path == tmp_path / "acme-corp" / CATALOGOS_DIR / "skus" / "2026-06-12_143052"
...
def test_ensure_load_storage_dir_creates_directories(tmp_path: Path):
    ...
    assert path.is_dir()
    assert (tmp_path / "demo-org" / CATALOGOS_DIR / "location").is_dir()


def test_resolve_batch_work_dir_uses_metadata(tmp_path: Path, monkeypatch):
    work = tmp_path / "org" / HISTORIA_DIR / "2026-06-12_120000"
...
def test_build_catalog_requires_catalog_name(tmp_path: Path):
    with pytest.raises(ValueError, match="catalog_name"):
        build_load_storage_dir("org", "catalog", upload_root=tmp_path)


def test_load_storage_metadata_fields(tmp_path: Path):
    ts = datetime(2026, 6, 12, 14, 30, 52)
    storage = tmp_path / "org" / HISTORIA_DIR / "2026-06-12_143052"
    storage.mkdir(parents=True)
    fields = load_storage_metadata_fields(storage, "org", ts)
    assert fields["organization_slug"] == "org"
